...
**MKQ AI V4 for Copilot Chat** 是一个 VS Code 插件，将 MKQ AI V4 Pro 和 Flash 直接添加到 GitHub Copilot 的模型选择器中。你仍可使用 Copilot 的 Agent 模式、工具调用、Skills 和 MCP — 全部由 MKQ AI 驱动。
...
- 点击聊天面板右上角的模型选择器。
- 选择 **MKQ AI V4 Pro** 或 **MKQ AI V4 Flash**。
- 即可开始对话 — Agent 模式、工具调用及所有 Copilot 功能均可直接使用。

#### 可选：配置思考深度

在模型选择器中，点击 MKQ AI 模型旁的齿轮图标可选择思考深度：
- **None** — 最快，不启用推理。
- **High** — 平衡模式（默认）。
- **Max** — 深度推理，适合复杂任务。
...
MKQ AI V4 为纯文本模型，但插件会自动处理图片。将截图拖入对话后，插件会通过其他已安装的 Copilot 模型（如 Claude、GPT-4o）描述图片内容，再发送给 MKQ AI。执行 **DeepSeek: Set Vision Proxy Model** 可选择用于图片描述的模型。
